// Comentário para forçar deploy v26 - FINAL SEM CHATBOT
...
window.addEventListener('load', () => {
    ...
    if (saudacaoTitulo) {
        ...
        if(saudacaoTitulo.textContent.includes(textoOriginal)) { // Evita re-escrever
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (nomeDisplay) { // Estamos na página arquivo.html
        ...
        if(btnAtualizar && novaLocalizacaoInput){
            btnAtualizar.addEventListener('click', function() { /* ... (código atualizar local) ... */ });
        }
